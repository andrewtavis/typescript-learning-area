window.onload = function (): void {
  ...
  function init() {
    ...
    for (let i = 0; i < videos.length; i++) {
      ...
      let objectStore = db
        .transaction("videos")
        .objectStore("videos") as IDBObjectStore;
      let request = objectStore.get(videos[i].name) as IDBRequest<any>;
  ...
  function fetchVideoFromNetwork(video: { name: string }) {
    console.log("fetching videos from network");
    // Fetch the MP4 and WebM versions of the video using the fetch() function,
    // then expose their response bodies as blobs
    let mp4Blob: Promise<Blob> = fetch(
      "videos/" + video.name + ".mp4"
    ).then((response) => response.blob());
    let webmBlob: Promise<Blob> = fetch(
      "videos/" + video.name + ".webm"
    ).then((response) => response.blob());

    // Only run the next code when both promises have fulfilled
    Promise.all([mp4Blob, webmBlob]).then(function (values: any[]) {
  ...
  function storeVideo(
    mp4Blob: Promise<Blob>,
    webmBlob: Promise<Blob>,
    name: string
  ) {
